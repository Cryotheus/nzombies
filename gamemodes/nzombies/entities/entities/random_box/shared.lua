...
function ENT:SetupDataTables() self:NetworkVar("Bool", 0, "Open") end
...
function ENT:Initialize()
	...
	if SERVER then self:SetUseType(SIMPLE_USE)
	else
...
function ENT:Think()
	...
	if self.MarkedForRemoval and not self:GetOpen() then self:Remove() end
...
function ENT:MoveAway()
	...
	--the box's shaking
	timer.Create("shake_" .. self:EntIndex(), 0.1, 24, function()
		if s < 23 and self:IsValid() then
			if s % 2 == 0 then self:SetAngles(ang + Angle(10, 0, 0))
			else self:SetAngles(ang + Angle(-10, 0, 0)) end
		else
			self:SetAngles(ang)
	...
	timer.Simple( 1, function()
		timer.Create("moveAway", 5, 1, function()
			...
			timer.Destroy("shake_" .. self:EntIndex())

			self.SpawnPoint.Box = nil
			--self.SpawnPoint:SetBodygroup(1, 0) --so it had a body group? neat
			self:MoveToNewSpot(self.SpawnPoint)
			self:EmitSound("nz/randombox/poof.wav") --IT'S GETTING REMOVED DUDE
			self:Remove()
		end)
