...
if SERVER then
	...
	function nzRevive:SendPlayerBeingRevived(ply, revivor, receiver)
		...
		if IsValid(revivor) then
			...
			--no valid revivor means the player stopped being revived
			net.WriteBool(false)
		end
...
if CLIENT then
	local function ReceivePlayerDowned()
		...
		for k, v in pairs(attached) do nzRevive.Players[id][k] = v end
		
		if IsValid(ply) and ply:IsPlayer() then nzRevive:DownedHeadsUp(ply, "needs to be revived!") end
	end
	...
	local function ReceivePlayerBeingRevived()
		...
		if bool then
			...
			if not nzRevive.Players[id].ReviveTime then
	...
	local function ReceivePlayerKilled()
		...
		if id and nzRevive.Players[id] then
			local revivor = nzRevive.Players[id].RevivePlayer
	...
	local function ReceiveFullPlayerSync()
		...
		if IsValid(ply) and ply:IsPlayer() then nzRevive:DownedHeadsUp(ply, "has been downed!") end
	end
