...
nzRound:AddBossType("Panzer", "nz_zombie_boss_panzer", {
	specialspawn = true,
	initfunc = function()
		nzRound:SetNextBossRound(7) -- Randomly spawn in round 7
	end,
	spawnfunc = function(panzer)
		panzer:SetHealth(nzRound:GetNumber() * 75 + 500)
	end,
	deathfunc = function(panzer, attacker, dmginfo, hitgroup)
		nzRound:SetNextBossRound(nzRound:GetNumber() + 3) -- Delay further boss spawning by 3 rounds after its death
		if IsValid(attacker) and attacker:IsPlayer() and attacker:GetNotDowned() then
			attacker:GivePoints(500) -- Give attacker 500 points if not downed
		end
	end,
