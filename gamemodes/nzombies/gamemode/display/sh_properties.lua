...
properties.Add("nz_remove", {
	...
	Filter = function(self, ent, ply) -- A function that determines whether an entity is valid for this property
		if not nzRound:InState(ROUND_CREATE) then return false end
	...
	Receive = function(self, length, player)
		...
		constraint.RemoveAll( ent ) --remove all constraint, this stops ropes from hanging around
		timer.Simple(1, function() if IsValid(ent) then ent:Remove() end end) --remove it properly in 1 second
		
		ent:SetNotSolid(true) --make it non solid \/
		ent:SetMoveType(MOVETYPE_NONE)
		ent:SetNoDraw(true)
		
		--send the entity remove sparks effect
		local ed = EffectData()
...
properties.Add("nz_editentity", {
	...
	Action = function(self, ent)
		...
		control.OnEntityLost = function() window:Remove() end
	end
})
...
properties.Add("nz_lock", {
	...
	Filter = function(self, ent, ply)
		...
		if not ent:IsDoor() or ent:IsButton() or ent:IsBuyableProp() then return false end
		if not nzRound:InState(ROUND_CREATE) then return false end
		if not ply:IsInCreative() then return false end
	...
	Action = function(self, ent)
		...
		panel.UpdateData = function(data) data2 = data end
...
properties.Add("nz_unlock", {
	...
	Filter = function(self, ent, ply)
		...
		if not nzRound:InState(ROUND_CREATE) then return false end
		if not ply:IsInCreative() then return false end
		if ent:IsBuyableProp() then if not nzDoors.PropDoors[ent:EntIndex()] then return false end
		elseif not nzDoors.MapDoors[ent:DoorIndex()] then return false end
...
properties.Add("nz_nocollide_off", {
	...
	Filter = function(self, ent, ply)
		if not IsValid(ent) then return false end
		if not ent:GetClass() ~= "prop_buys" then return false end
		if not nzRound:InState(ROUND_CREATE) then return false end
		if not ply:IsAdmin() then return false end
